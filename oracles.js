...
export function shuffleArray(arr, n) {
    for (let i = 0; i < n; i++) {
 ...
 */
function stableMatch(hires, companies, candidates, N) {
    function preferred(C, pick, actualPick) {
        let i = 0;
    ...
    function searchPairing(preferredCandidate) {
        let j = 0;
...
function checkDuplicateCandidates(hires) {
...
function checkMissingCandidates(hires, N) {
    for (let x = 0; x < hires.length - 1; ++x) {
...
class C {
    ...
    proposals;
    index;
    constructor(preferences, index) {
        this.preferences = preferences;
        this.Match = -1;
        this.proposals = [];
        this.index = index;
    }
    receiveProposal(ProposerIndex, C) {
...
function preferencesObject(CompCan) {
    return CompCan.map((x, y) => new C(x, y));
}
function forceMatch(ProposerObj, proposed) {
    ProposerObj.Match = proposed;
}
function storeProposals(offers, companies, candidates) {
...
function CheckProposals(out, companies, candidates, trace) {
    ...
    for (let x = 0; x < companyObj.length; x++) {
        ...
        if (new Set(listProposals).size !== listProposals.length)
            return assert(false, "duplicates");
...
function checkMatches(out, companies, candidates) {
